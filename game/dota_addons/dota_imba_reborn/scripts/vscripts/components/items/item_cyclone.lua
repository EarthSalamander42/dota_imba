...
LinkLuaModifier("modifier_item_imba_cyclone_2_movement", "components/items/item_cyclone", LUA_MODIFIER_MOTION_HORIZONTAL)
...
function modifier_item_imba_cyclone_2_movement:OnCreated()
	self.self_movement_speed 			= self:GetAbility():GetSpecialValueFor("self_movement_speed")
	self.centrifugal_damage_per_tick	= self:GetAbility():GetSpecialValueFor("centrifugal_damage_per_tick")
	self.centrifugal_tick_rate			= self:GetAbility():GetSpecialValueFor("centrifugal_tick_rate")
...
function modifier_item_imba_cyclone_2_movement:OnIntervalThink()
	self:GetParent():SetAbsOrigin(self:GetParent():GetAbsOrigin() + self:GetParent():GetForwardVector() * RandomInt(200, 300))
end


-- function modifier_item_imba_cyclone_2_movement:UpdateHorizontalMotion(me, dt)
	-- me:SetAbsOrigin(me:GetAbsOrigin() + self:GetParent():GetForwardVector() * self.self_movement_speed * 1000 * dt)
-- end

-- -- This typically gets called if the caster uses a position breaking tool (ex. Blink Dagger) while in mid-motion
-- function modifier_item_imba_cyclone_2_movement:OnHorizontalMotionInterrupted()
	-- self:Destroy()
-- end
...
function modifier_item_imba_cyclone_2_movement:OnDestroy()
	...
	if self:GetParent():GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then
		self:GetParent():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_item_imba_cyclone_2_disorient", {duration = 4})
	end
end
...
function modifier_item_imba_cyclone_2_thinker:OnCreated()
...
function modifier_item_imba_cyclone_2_thinker:GetAuraRadius()			return 500 end
...
-- function modifier_item_imba_cyclone_2_thinker:GetAuraEntityReject(hTarget)	
	-- if not IsServer() then return end
-- end
...
-- function item_imba_valiance:GetBehavior()
	-- if not self:GetCaster():HasModifier("modifier_item_imba_valiance_counter") and not self:GetCaster():HasModifier("modifier_item_imba_valiance_guard") then
		-- return DOTA_ABILITY_BEHAVIOR_NO_TARGET + DOTA_ABILITY_BEHAVIOR_OPTIONAL_POINT + DOTA_ABILITY_BEHAVIOR_IGNORE_BACKSWING
	-- else
...
-- function modifier_item_imba_valiance_counter:OnDestroy()
	...
	-- if self:GetAbility() then
		-- self:GetAbility():StartCooldown(self:GetAbility():GetEffectiveCooldown(self:GetAbility():GetLevel()) - self:GetElapsedTime())
	-- end
-- end
