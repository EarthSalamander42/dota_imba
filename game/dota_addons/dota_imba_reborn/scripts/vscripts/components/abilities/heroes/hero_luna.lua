...
LinkLuaModifier("modifier_imba_luna_lucent_beam_synchronicity", "components/abilities/heroes/hero_luna", LUA_MODIFIER_MOTION_NONE)

LinkLuaModifier("modifier_imba_luna_moon_glaive", "components/abilities/heroes/hero_luna", LUA_MODIFIER_MOTION_NONE)

LinkLuaModifier("modifier_imba_luna_lunar_blessing", "components/abilities/heroes/hero_luna", LUA_MODIFIER_MOTION_NONE)
...
modifier_imba_luna_lucent_beam_synchronicity	= class({})
...
function imba_luna_lucent_beam:OnSpellStart()
	...
	local random_vector_1	= RandomVector(200)
	local random_vector_2	= RandomVector(600)
	
	Timers:CreateTimer(0.4, function()
		for inner_beam = 1, 4 do
			local beam_pos = RotatePosition(target_pos, QAngle(0, 360 * inner_beam / 4, 0), target_pos + random_vector_1)
			local particle = ParticleManager:CreateParticle("particles/units/heroes/hero_luna/luna_lucent_beam.vpcf", PATTACH_POINT_FOLLOW, self:GetCaster())
	...
	Timers:CreateTimer(0.8, function()
		for outer_beam = 1, 8 do
			local beam_pos = RotatePosition(target_pos, QAngle(0, 360 * outer_beam / 8, 0), target_pos + random_vector_2)
			local particle = ParticleManager:CreateParticle("particles/units/heroes/hero_luna/luna_lucent_beam.vpcf", PATTACH_POINT_FOLLOW, self:GetCaster())
			ParticleManager:SetParticleControl(particle, 1, beam_pos)
			ParticleManager:SetParticleControl(particle, 5, beam_pos)
			ParticleManager:SetParticleControlEnt(particle,	6, self:GetCaster(), PATTACH_POINT_FOLLOW, "attach_attack1", self:GetCaster():GetAbsOrigin(), true)
...
----------------------------------------
-- LUCENT BEAM SYNCHRONICITY MODIFIER --
----------------------------------------
...
-- No dice
-- function modifier_imba_luna_moon_glaive:GetEffectName()
	-- return "particles/units/heroes/hero_luna/luna_ambient_moon_glaive.vpcf"
-- end
...
function imba_luna_lunar_blessing:GetIntrinsicModifierName()
	return "modifier_imba_luna_lunar_blessing"
end

-----------------------------
-- LUNAR BLESSING MODIFIER --
-----------------------------

function modifier_imba_luna_lunar_blessing:IsHidden()					return not (self:GetAbility() and self:GetAbility():GetLevel() >= 1) end

function modifier_imba_luna_lunar_blessing:IsAura()						return self:GetParent() == self:GetCaster() end
function modifier_imba_luna_lunar_blessing:IsAuraActiveOnDeath() 		return false end

function modifier_imba_luna_lunar_blessing:GetAuraRadius()				return self:GetAbility():GetSpecialValueFor("radius") end
function modifier_imba_luna_lunar_blessing:GetAuraSearchFlags()			return DOTA_UNIT_TARGET_FLAG_INVULNERABLE + DOTA_UNIT_TARGET_FLAG_OUT_OF_WORLD end
...
function modifier_imba_luna_lunar_blessing:GetModifierAura()			return "modifier_imba_luna_lunar_blessing" end
function modifier_imba_luna_lunar_blessing:GetAuraEntityReject(hEntity)	return hEntity == self:GetCaster() end -- Already an intrinsic modifier

function modifier_imba_luna_lunar_blessing:GetEffectName()
	if self:GetAbility() and self:GetAbility():GetLevel() >= 1 and self:GetParent() == self:GetCaster() then return "particles/units/heroes/hero_luna/luna_ambient_lunar_blessing.vpcf" end
end
...
function modifier_imba_luna_lunar_blessing:DeclareFunctions()
	local decFuncs = {
...
function modifier_imba_luna_lunar_blessing:GetModifierBonusStats_Strength()
	if self:GetAbility() and self.hero_primary_attribute == DOTA_ATTRIBUTE_STRENGTH and not self:GetCaster():PassivesDisabled() then
		return self:GetAbility():GetSpecialValueFor("primary_attribute")
	end
end

function modifier_imba_luna_lunar_blessing:GetModifierBonusStats_Agility()
	if self:GetAbility() and self.hero_primary_attribute == DOTA_ATTRIBUTE_AGILITY and not self:GetCaster():PassivesDisabled() then
		return self:GetAbility():GetSpecialValueFor("primary_attribute")
	end
end

function modifier_imba_luna_lunar_blessing:GetModifierBonusStats_Intellect()
	if self:GetAbility() and self.hero_primary_attribute == DOTA_ATTRIBUTE_INTELLECT and not self:GetCaster():PassivesDisabled() then
		return self:GetAbility():GetSpecialValueFor("primary_attribute")
	end
end

function modifier_imba_luna_lunar_blessing:GetBonusNightVision()
	if self:GetAbility() and self:GetParent() == self:GetCaster() and not self:GetCaster():PassivesDisabled() then
...
function imba_luna_eclipse:OnSpellStart()
	...
	local eclipse_particle = ParticleManager:CreateParticle("particles/units/heroes/hero_luna/luna_eclipse.vpcf", PATTACH_POINT_FOLLOW, self:GetCaster())
	ParticleManager:SetParticleControl(eclipse_particle, 1, Vector(self:GetSpecialValueFor("radius"), 0, 0))
...
function modifier_imba_luna_eclipse:RemoveOnDeath()	return false end -- IMBAfication:
...
function modifier_imba_luna_eclipse:OnCreated(params)
	...
	if params.pos then
		self.target_position = params.pos
	end
...
function modifier_imba_luna_eclipse:OnIntervalThink()
	...
	-- The Eclipse modifier itself doesn't have an actual buff duration; it removes itself after a number of beams have fired
	if (self:GetCaster():HasScepter() and self.beams_elapsed >= self.beams_scepter) or self.beams_elapsed >= self.beams then
		self:Destroy()
		return
	end
	...
	for _, enemy in pairs(enemies) do
		...
		if (self:GetCaster():HasScepter() and self.targets[enemy:GetEntityIndex()] < self.hit_count_scepter) or self.targets[enemy:GetEntityIndex()] < self.hit_count then
			self.targets[enemy:GetEntityIndex()] = self.targets[enemy:GetEntityIndex()] + 1

			enemy:EmitSound("Hero_Luna.Eclipse.Target")

			local particle = ParticleManager:CreateParticle("particles/units/heroes/hero_luna/luna_eclipse_impact.vpcf", PATTACH_POINT_FOLLOW, self:GetCaster())
			ParticleManager:SetParticleControl(particle, 1, enemy:GetAbsOrigin())
			ParticleManager:SetParticleControlEnt(particle,	5, enemy, PATTACH_POINT_FOLLOW, "attach_hitloc", enemy:GetAbsOrigin(), true)
			ParticleManager:ReleaseParticleIndex(particle)
	...
	if not valid_targets then
		...
		local particle = ParticleManager:CreateParticle("particles/units/heroes/hero_luna/luna_loadout.vpcf", PATTACH_POINT_FOLLOW, self:GetCaster())
