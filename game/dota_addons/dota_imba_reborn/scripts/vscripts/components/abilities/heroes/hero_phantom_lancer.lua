...
-- modifier_imba_phantom_lancer_juxtapose	= class({})
...
-- function modifier_imba_phantom_lancer_juxtapose:OnAttackLanded(params)
	-- if not IsServer() then return end
	
	-- if params.attacker == self:GetParent() and not self:GetParent():PassivesDisabled() and not params.target:IsBuilding() and ((self:GetParent():IsRealHero() and RollPseudoRandom(self:GetAbility():GetSpecialValueFor("proc_chance_pct"), self)) or ((not self:GetParent():IsRealHero() and RollPseudoRandom(self:GetAbility():GetSpecialValueFor("illusion_proc_chance_pct"), self)))) then
		-- local spirit_lance_ability = self:GetCaster():FindAbilityByName("phantom_lancer_spirit_lance")
		-- local spirit_lance_passive = self:GetCaster():FindAbilityByName("imba_phantom_lancer_spirit_lance_passive")

		-- if spirit_lance_ability and spirit_lance_ability:IsTrained() and spirit_lance_passive then
			-- if spirit_lance_ability:GetLevel() ~= spirit_lance_passive:GetLevel() then
				-- spirit_lance_passive:SetLevel(spirit_lance_ability:GetLevel())
			-- end
			
			-- self:GetParent():CastAbilityOnTarget(params.target, spirit_lance_passive, self:GetParent():GetPlayerID())
		-- end
	-- end
-- end
