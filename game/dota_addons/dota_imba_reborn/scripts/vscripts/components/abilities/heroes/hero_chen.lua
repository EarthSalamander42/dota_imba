...
function imba_chen_holy_persuasion:OnSpellStart()
	...
	if target:GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then			
		...
		local commonwealth_xp			= target_xp * self:GetSpecialValueFor("commonwealth_pct") * 0.01
		local commonwealth_gold			= RandomInt(target_gold_min, target_gold_max) * self:GetSpecialValueFor("commonwealth_pct") * 0.01
		
		local commonwealth_xp_self		= commonwealth_xp * self:GetSpecialValueFor("commonwealth_caster_pct") * 0.01
		local commonwealth_gold_self	= commonwealth_gold * self:GetSpecialValueFor("commonwealth_caster_pct") * 0.01
