...
local function getOverChannelMistIncrease(caster)
	if caster:HasModifier("modifier_over_channel_handler") then
		...
		if over_channel then
			...
			-- #6 Talent: +50% Overchannel power
			return over_channel:GetSpecialValueFor("extra_mist_duration") * (1 + caster:FindTalentValue("special_bonus_imba_abaddon_6"))
		end
	end
...
local function getOverChannelShieldIncrease(caster)
	if caster:HasModifier("modifier_over_channel_handler") then
		...
		if over_channel then
			local ability_level = over_channel:GetLevel() - 1
			-- #6 Talent: +50% Overchannel power
			return over_channel:GetSpecialValueFor("extra_dmg") * (1 + caster:FindTalentValue("special_bonus_imba_abaddon_6")) --+ over_channel:GetLevelSpecialValueFor( "extra_shield_health" , ability_level )
		end
	end
...
function imba_abaddon_death_coil:OnProjectileHit_ExtraData( hTarget, vLocation, ExtraData)
	if IsServer() then
		...
		if target:GetTeam() ~= caster:GetTeam() then
			...
			local damage = self:GetSpecialValueFor("target_damage") + self.overchannel_damage_increase
			...
			if curse_of_avernus then
				local debuff_duration = curse_of_avernus:GetSpecialValueFor("slow_duration")
			...
			local heal = (self:GetSpecialValueFor("heal_amount") + self.overchannel_damage_increase) -- * heal_amp
...
function modifier_imba_aphotic_shield_buff_block:OnCreated()
	if IsServer() then
		...
		self.shield_init_value = ability:GetSpecialValueFor("damage_absorb") + getOverChannelShieldIncrease(caster)
...
function imba_abaddon_frostmourne:IsStealable() return false end
...
function modifier_imba_curse_of_avernus_debuff_counter:OnRefresh(kv)
	...
	if self:GetStackCount() >= self:GetAbility():GetSpecialValueFor("hit_count") then
		self:GetParent():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_imba_curse_of_avernus_debuff_slow", {duration = self:GetAbility():GetSpecialValueFor("curse_duration")})
...
function modifier_imba_curse_of_avernus_debuff_counter:GetModifierMoveSpeedBonus_Percentage() return self:GetAbility():GetSpecialValueFor("movement_speed") * (-1) end
...
function modifier_imba_curse_of_avernus_debuff_slow:CheckState() return {
