...
function modifier_imprisoned_soldier:TeleportOut()
	print( "try teleport out:", self:GetParent():GetUnitName() )
...
--		print( "found a tower:", teleport:GetName() )
--		local item = self:GetParent():FindItemInInventory("item_tpscroll")

--		if item then
--			if not item:IsCooldownReady() then
--				print("TP scroll in cd, end it")
--				item:EndCooldown()
--			end

--			print( "Execute order (tp scroll)" )
