...
function CDungeon:OnNPCSpawned_PlayerHero( event )
	...
	if hPlayerHero ~= nil then
		...
		if hPlayerHero.bFirstSpawnComplete == nil then
			hPlayerHero:SetStashEnabled( STASH_ENABLED )
			...
			if GetMapName() == "ep_2" and not PlayerResource:IsFakeClient( hPlayerHero:GetPlayerOwnerID() ) then
...
--			else
				hPlayerHero.nRespawnsRemaining = nSTARTING_RESPAWNS[GameRules:GetCustomGameDifficulty()]
--			end
...
--		end
		...
		hEnemyCreature:SetMinimumGoldBounty(hEnemyCreature:GetGoldBounty() * (CustomNetTables:GetTableValue("game_options", "gold_multiplier")[tostring(GameRules:GetCustomGameDifficulty())] - 0.1))
		hEnemyCreature:SetMaximumGoldBounty(hEnemyCreature:GetGoldBounty() * (CustomNetTables:GetTableValue("game_options", "gold_multiplier")[tostring(GameRules:GetCustomGameDifficulty())] + 0.1))
		hEnemyCreature:SetDeathXP(hEnemyCreature:GetDeathXP() * CustomNetTables:GetTableValue("game_options", "xp_multiplier")[tostring(GameRules:GetCustomGameDifficulty())])
		hEnemyCreature:SetBaseDamageMin(hEnemyCreature:GetBaseDamageMin() * CustomNetTables:GetTableValue("game_options", "damage_multiplier")[tostring(GameRules:GetCustomGameDifficulty())])
		hEnemyCreature:SetBaseDamageMax(hEnemyCreature:GetBaseDamageMax() * CustomNetTables:GetTableValue("game_options", "damage_multiplier")[tostring(GameRules:GetCustomGameDifficulty())])
		hEnemyCreature:SetBaseMaxHealth(hEnemyCreature:GetMaxHealth() * CustomNetTables:GetTableValue("game_options", "damage_multiplier")[tostring(GameRules:GetCustomGameDifficulty())])
		hEnemyCreature:SetMaxHealth(hEnemyCreature:GetMaxHealth() * CustomNetTables:GetTableValue("game_options", "damage_multiplier")[tostring(GameRules:GetCustomGameDifficulty())])
		hEnemyCreature:SetHealth(hEnemyCreature:GetMaxHealth())

		for i = 0, 24 do
			if hEnemyCreature.GetAbilityByIndex and hEnemyCreature:GetAbilityByIndex(i) then
				local ability = hEnemyCreature:GetAbilityByIndex(i)

				if not ability:GetAbilityName() == "creature_techies_land_mine" and not ability:GetAbilityName() == "trap_sun_strike" then
					ability:SetLevel(GameRules:GetCustomGameDifficulty())
				end
			end
		end
	end
...
function CDungeon:OnScrollClicked( eventSourceIndex, data )
	print("CDungeon:OnScrollClicked")
	...
	if hPlayerHero ~= nil and hPlayerHero:FindModifierByName( "modifier_pre_teleport" ) ~= nil then
		print("Hero have pre teleport modifier, check if it have a TP...")
		if hPlayerHero:FindItemInInventory("item_tpscroll") then
			local item = hPlayerHero:FindItemInInventory("item_tpscroll")

			if item:GetCooldownTimeRemaining() > 0 then
				print("TP was on cooldown, ended it")
				item:EndCooldown()
			end

			print("Execute tp order...")
