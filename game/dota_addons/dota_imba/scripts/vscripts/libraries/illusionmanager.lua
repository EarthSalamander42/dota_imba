...
Keep in mind illusions are nothing more than painted hero baseclasses, and the modifiers do the heavy lifting for all the things illus should/should not do.
]]--
...
modifier_illusion_manager = class({
		...
		funcs = {MODIFIER_PROPERTY_TOTALDAMAGEOUTGOING_PERCENTAGE,MODIFIER_PROPERTY_INCOMING_DAMAGE_PERCENTAGE,MODIFIER_EVENT_ON_TAKEDAMAGE},
		states = {}
		})		
...
function IllusionManager:MoveExistingIllusion(tEntity,tIllusionBase,tSkill,vSpawnLocation,tFoundIllusion,tSpecialKeys)
	...
	tFoundIllusion:Stop()
	tFoundIllusion:SetHealth(99999)
...
function IllusionManager:ResetIllusion(tEntity,tIllusion)  -- DELETE entity from the game
	if not IsServer() then return end	
	...
	for item_slot = 0, 5 do
		...
		if individual_item ~= nil then
			...
			tIllusion:AddItem(illusion_duplicate_item)
		end
	end	
end

function IllusionManager:WipeIllusion(tIllusion)  -- DELETE entity from the game
	if not IsServer() then return end	
	if tIllusion:IsCreep() then return end
	for ability_slot = 0, 15 do
...
function IllusionManager:SetModifiers(tEntity,tIllusionBase,tIllusion,tSkill,tSpecialKeys)
	...
	IllusionManager:ResetIllusion(tEntity,tIllusion)
