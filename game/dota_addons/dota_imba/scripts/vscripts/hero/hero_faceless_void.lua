...
function Chronosphere( keys )
	...
	local total_duration = ability:GetLevelSpecialValueFor("base_duration", ability_level)
	...
	-- Effect loop
	local elapsed_duration = 0
	local remaining_duration = total_duration
	Timers:CreateTimer(0, function()
		...
		for _,unit in pairs(units) do
			...
			elseif scepter and unit:GetTeam() == caster:GetTeam() then
				...
				if not unit:HasModifier(modifier_enemy) then
					ability:ApplyDataDrivenModifier(caster, unit, modifier_enemy, {duration = remaining_duration})
				end
				if not unit:HasModifier("modifier_faceless_void_chronosphere_freeze") then
					ability:ApplyDataDrivenModifier(caster, unit, "modifier_faceless_void_chronosphere_freeze", {duration = remaining_duration})
				end
			end
		end
