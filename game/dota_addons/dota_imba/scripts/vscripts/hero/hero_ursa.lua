	...
	if IsServer() then
		...
		for _,enemy in pairs(enemies) do
			...
			if not enemy:IsMagicImmune() then
				...
				local damage = nil --initialize						
				
				-- If enemy is closer than 190 units, set bonuses, otherwise calculate damage/slow scaling with distance.
				if distance <= bonus_effects_radius then
					damage = base_damage * (1 + bonus_damage_pct)
					enemy.earthshock_debuff_slow_pct = slow_pct + bonus_slow_pct					
				else
					...
					enemy.earthshock_debuff_slow_pct = slow_pct * (1+ pct_increase_for_distance) -- final slow value									
				end				
...
function modifier_imba_earthshock_slow:GetModifierMoveSpeedBonus_Percentage()	
	...
	return enemy.earthshock_debuff_slow_pct
end
...
function modifier_imba_trembling_steps_debuff:GetModifierMoveSpeedBonus_Percentage()	
	...
	return trembling_steps_slow_pct
end
...
function modifier_imba_overpower_buff:OnAttack( keys)
	if keys.attacker == self:GetCaster() then
		...
		local buff_name = self:GetName()
		
		if current_stacks > 1 then
			caster:SetModifierStackCount(buff_name, caster, current_stacks - 1)
		else	
...
function imba_ursa_fury_swipes:OnUpgrade()
	 local caster = self:GetCaster()
	 local ability = self
	 local fury_swipes_modifier = "modifier_imba_fury_swipes"
	 
	 if not caster:HasModifier(fury_swipes_modifier) then
		caster:AddNewModifier(caster, ability, fury_swipes_modifier, {})
	 end
end
...
function modifier_imba_fury_swipes_debuff:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
	...
	if IsServer() then
		...
		if keys.attacker == caster then	
			...
			if fury_swipes_stacks % deep_stack_attacks == 0 then --divides with no remainder
				damage = damage * deep_stack_multiplier
...
function imba_ursa_enrage:GetCooldown()
	local caster = self:GetCaster()
	local ability = self
