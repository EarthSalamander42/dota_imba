...
function imba_vengefulspirit_nether_swap:OnOwnerDied()
	...
	if self:GetLevel() > 0 and caster:HasScepter() and( not caster:IsIllusion() ) then
		...
		super_illusion:AddNewModifier(caster, self, "modifier_illusion", {outgoing_damage = self:GetSpecialValueFor("tooltip_illu_dmg_scepter"), incoming_damage = self:GetSpecialValueFor("tooltip_illu_amp_scepter")})
