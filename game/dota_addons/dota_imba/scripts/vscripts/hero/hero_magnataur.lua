...
function modifier_imba_empower_polarizer:OnIntervalThink()
	if IsServer() then
		...
		-- Find enemies to polarize
		local empower_search = FindUnitsInRadius(self.parent:GetTeamNumber(), self.parent:GetAbsOrigin(), nil, self.search_radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_NONES, FIND_ANY_ORDER, false)
		local refresh_interval = self.ability:GetSpecialValueFor("refresh_interval")
