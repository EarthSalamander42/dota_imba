...
function item_cavern_treasure:OnSpellStart()
	if IsServer() then
		...
		if #HeroesToReward >= 1 then
			...
			if bBigCheese then
				CCavern:OnBigCheeseTaken( self:GetCaster():GetTeamNumber() )
				CCavern:OnBattlePointsEarned( self:GetCaster():GetTeamNumber(), nBattlePointAmount, "points_big_cheese" )
			else
				CCavern:OnBattlePointsEarned( self:GetCaster():GetTeamNumber(), nBattlePointAmount, "points_small_cheese" )
			end
		end
