...
function CopyTalents(fromEntity, toEntity, type) --type 1(generic only), 2(unique only), 3(all)
    local copyGeneric = ((type == 1) or (type == 3))
    local copyUnique = ((type == 2) or (type == 3))

    if copyGeneric then
        local modifierList = fromEntity:FindAllModifiers()
        for _,modifier in pairs(modifierList) do
            local modifierName = modifier:GetName()
            -- Check if it is a generic talent
            if IMBA_GENERIC_TALENT_LIST[modifierName] ~= nil then
                -- Apply to entity
                local newModifier = toEntity:AddNewModifier(toEntity, nil, modifierName, {})
                if newModifier then
                    newModifier:SetStackCount(modifier:GetStackCount())
                    newModifier:ForceRefresh()
                else
                    print("failed to attach generic talent: "..modifier_name)
                end
            end
        end
    end
    ...
    if copyUnique then
        local endAbilityIndex = (fromEntity:GetAbilityCount()-1)

        while endAbilityIndex >= 0 do
            local ability = fromEntity:GetAbilityByIndex(endAbilityIndex)
            if ability then
                ...
                if abilityName:find("special_bonus_imba_") == 1 or abilityName:find("special_bonus_unique_") == 1 then
                    local newAbility = toEntity:AddAbility(abilityName)
                    if newAbility then
