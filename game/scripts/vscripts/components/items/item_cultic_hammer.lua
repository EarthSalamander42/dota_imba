...
function item_imba_cultic_hammer:OnChannelFinish(bInterrupted)
	...
	if not bInterrupted then
		...
		for _, enemy in pairs(FindUnitsInRadius(self:GetCaster():GetTeamNumber(), self:GetCursorPosition(), nil, self:GetSpecialValueFor("wretched_radius"), DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_NONE, FIND_ANY_ORDER, false)) do
			health_loss = enemy:GetHealth() * self:GetSpecialValueFor("wretched_hp_damage_pct") / 100
...
function modifier_item_imba_cultic_hammer_aura:OnCreated()
	...
	self.soul_drain_health_illusions = self.soul_drain_health * self:GetAbility():GetSpecialValueFor("soul_drain_illusion_efficiency") / 100
	self.soul_drain_mana_illusions   = self.soul_drain_mana * self:GetAbility():GetSpecialValueFor("soul_drain_illusion_efficiency") / 100
