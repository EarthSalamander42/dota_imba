...
--	EarthSalamander, 05/11/2021

---------------------------------
-- 		   Thick Hide          --
---------------------------------

imba_centaur_thick_hide = imba_centaur_thick_hide or class({})

LinkLuaModifier("modifier_imba_thick_hide", "components/abilities/heroes/centaur.lua", LUA_MODIFIER_MOTION_NONE)
...
-- Thick hide modifier
modifier_imba_thick_hide = modifier_imba_thick_hide or class({})
...
imba_centaur_hoof_stomp = imba_centaur_hoof_stomp or class(VANILLA_ABILITIES_BASECLASS)

LinkLuaModifier("modifier_imba_hoof_stomp_arena_thinker_buff", "components/abilities/heroes/centaur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_hoof_stomp_arena_thinker_debuff", "components/abilities/heroes/centaur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_hoof_stomp_arena_debuff", "components/abilities/heroes/centaur.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_hoof_stomp_arena_buff", "components/abilities/heroes/centaur.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_hoof_stomp_arena_debuff", "components/abilities/heroes/centaur.lua", LUA_MODIFIER_MOTION_NONE)
...
function imba_centaur_hoof_stomp:OnSpellStart()
	...
	ParticleManager:SetParticleControl(particle_stomp_fx, 1, Vector(self:GetVanillaAbilitySpecial("radius"), 1, 1))
	...
	for _, enemy in pairs(FindUnitsInRadius(self:GetCaster():GetTeamNumber(), self:GetCaster():GetAbsOrigin(), nil, self:GetVanillaAbilitySpecial("radius"), DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_ANY_ORDER, false)) do
		...
		if not enemy:IsMagicImmune() then
			...
			-- Stun them
			enemy:AddNewModifier(self:GetCaster(), self, "modifier_stunned", {duration = self:GetVanillaAbilitySpecial("stun_duration") * (1 - enemy:GetStatusResistance())})
			
			-- Deal damage to nearby non-magic immune enemies
			ApplyDamage({
				victim 			= enemy,
				damage 			= self:GetVanillaAbilitySpecial("stomp_damage"),
...
-- Arena buff
modifier_imba_hoof_stomp_arena_buff = modifier_imba_hoof_stomp_arena_buff or class({})
...
modifier_imba_hoof_stomp_arena_debuff = modifier_imba_hoof_stomp_arena_debuff or class({})
...
imba_centaur_double_edge = imba_centaur_double_edge or class(VANILLA_ABILITIES_BASECLASS)

LinkLuaModifier("modifier_imba_centaur_doubleedge_buff", "components/abilities/heroes/centaur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_centaur_doubleedge_slow", "components/abilities/heroes/centaur", LUA_MODIFIER_MOTION_NONE)
...
function imba_centaur_double_edge:OnSpellStart()
	...
	local damage = ability:GetVanillaAbilitySpecial("edge_damage") + (caster:GetStrength() * caster:FindTalentValue("special_bonus_imba_centaur_4") * 0.01)
	...
	if caster:HasShard() then
		caster:AddNewModifier(caster, ability, "modifier_imba_centaur_doubleedge_buff", {duration = self:GetVanillaAbilitySpecial("shard_str_duration")})
		target:AddNewModifier(caster, ability, "modifier_imba_centaur_doubleedge_slow", {duration = self:GetSpecialValueFor("shard_slow_duration")})
	end
end

modifier_imba_centaur_doubleedge_buff = modifier_imba_centaur_doubleedge_buff or class({})

function modifier_imba_centaur_doubleedge_buff:DeclareFunctions() return {
	MODIFIER_PROPERTY_STATS_STRENGTH_BONUS,
} end

function modifier_imba_centaur_doubleedge_buff:OnCreated()
	if not IsServer() then return end

	self:SetStackCount(1)

	self.shard_pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_centaur/centaur_shard_buff_strength.vpcf", PATTACH_OVERHEAD_FOLLOW, self:GetParent())
--	ParticleManager:SetParticleControlEnt(self.shard_pfx, 0, self:GetParent(), PATTACH_OVERHEAD_FOLLOW, "follow_overhead", self:GetParent():GetAbsOrigin(), true)
...
function modifier_imba_centaur_doubleedge_buff:OnRefresh()
	if not IsServer() then return end

	self:SetStackCount(math.min(self:GetStackCount() + 1, self:GetAbility():GetSpecialValueFor("shard_max_stacks")))
	ParticleManager:SetParticleControl(self.shard_pfx, 2, Vector(self:GetStackCount(), 0, 0))
end

function modifier_imba_centaur_doubleedge_buff:GetModifierBonusStats_Strength()
	if self:GetAbility() then
		return self:GetParent():GetBaseStrength() * self:GetAbility():GetVanillaAbilitySpecial("shard_str_pct") / 100 * self:GetStackCount()
	end
end

modifier_imba_centaur_doubleedge_slow = modifier_imba_centaur_doubleedge_slow or class({})

function modifier_imba_centaur_doubleedge_slow:OnCreated()
	-- Ability properties
	self.caster = self:GetCaster()
	self.ability = self:GetAbility()

	-- Ability specials
	self.ms_slow_pct = self.ability:GetSpecialValueFor("shard_slow_movement_speed")
end

function modifier_imba_centaur_doubleedge_slow:DeclareFunctions()
	return {MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE}
end

function modifier_imba_centaur_doubleedge_slow:GetModifierMoveSpeedBonus_Percentage()
...
LinkLuaModifier("modifier_imba_return_aura", "components/abilities/heroes/centaur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_return_passive", "components/abilities/heroes/centaur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_return_damage_block", "components/abilities/heroes/centaur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_return_damage_block_buff", "components/abilities/heroes/centaur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_return_bonus_damage", "components/abilities/heroes/centaur", LUA_MODIFIER_MOTION_NONE)
...
-- Return Aura
modifier_imba_return_aura = modifier_imba_return_aura or class({})
...
function modifier_imba_return_aura:OnCreated()
	...
	-- Ability specials
	self.aura_radius = self.ability:GetVanillaAbilitySpecial("aura_radius")
end
...
-- Return modifier
modifier_imba_return_passive = modifier_imba_return_passive or class({})
...
function modifier_imba_return_passive:OnTakeDamage(keys)
	if IsServer() and self:GetAbility() then
		...
		-- Ability specials
		local damage = ability:GetVanillaAbilitySpecial("return_damage")
		local str_pct_as_damage = ability:GetVanillaAbilitySpecial("return_damage_str")
		...
		if attacker:GetTeamNumber() ~= parent:GetTeamNumber() and parent == target and not attacker:IsOther() and attacker:GetName() ~= "npc_dota_unit_undying_zombie" then
			...
			if self:GetParent().GetStrength then
				damage = damage + (self:GetParent():GetStrength() * self:GetAbility():GetSpecialValueFor("return_damage_str") * 0.01)
			end
...
modifier_imba_return_bonus_damage = modifier_imba_return_bonus_damage or class({})
...
imba_centaur_stampede = imba_centaur_stampede or class(VANILLA_ABILITIES_BASECLASS)

LinkLuaModifier("modifier_imba_stampede_haste", "components/abilities/heroes/centaur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_centaur_stampede_scepter", "components/abilities/heroes/centaur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_stampede_trample_slow", "components/abilities/heroes/centaur", LUA_MODIFIER_MOTION_NONE)
...
function imba_centaur_stampede:OnSpellStart()
	if IsServer() then
		...
		-- Ability specials
		local duration = ability:GetVanillaAbilitySpecial("duration")
		...
		for _,ally in pairs (allies) do
			...
			if self:GetCaster():HasScepter() then
				ally:AddNewModifier(self:GetCaster(), self, "modifier_imba_centaur_stampede_scepter", {duration = self:GetVanillaAbilitySpecial("duration")})
			end
		end
	end
end
...
-- Haste modifier
modifier_imba_stampede_haste = modifier_imba_stampede_haste or class({})
...
function modifier_imba_stampede_haste:OnCreated()
	...
	-- Ability specials
	self.strength_damage = self.ability:GetVanillaAbilitySpecial("strength_damage")
	self.stun_duration = self.ability:GetSpecialValueFor("stun_duration")
	self.radius = self.ability:GetVanillaAbilitySpecial("radius")
	self.damage_reduction_scepter = self.ability:GetVanillaAbilitySpecial("damage_reduction")
	self.absolute_move_speed = self.ability:GetSpecialValueFor("absolute_move_speed")
	self.slow_duration = self.ability:GetVanillaAbilitySpecial("slow_duration")
	...
	if IsServer() then
		...
		if self.caster:IsHero() then
			...
			self.trample_damage = self.caster:GetStrength() * self.strength_damage
		else
...
function modifier_imba_centaur_stampede_scepter:OnCreated()
	...
	self.damage_reduction_scepter	= self:GetAbility():GetVanillaAbilitySpecial("damage_reduction") * (-1)
end
...
modifier_imba_stampede_trample_slow = modifier_imba_stampede_trample_slow or class({})
...
function modifier_imba_stampede_trample_slow:OnCreated()
	...
	-- Ability specials
	self.ms_slow_pct = self.ability:GetVanillaAbilitySpecial("slow_movement_speed")
end
...
LinkLuaModifier("modifier_special_bonus_imba_centaur_1", "components/abilities/heroes/centaur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_special_bonus_imba_centaur_2", "components/abilities/heroes/centaur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_special_bonus_imba_centaur_3", "components/abilities/heroes/centaur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_special_bonus_imba_centaur_4", "components/abilities/heroes/centaur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_special_bonus_imba_centaur_5", "components/abilities/heroes/centaur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_special_bonus_imba_centaur_6", "components/abilities/heroes/centaur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_special_bonus_imba_centaur_7", "components/abilities/heroes/centaur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_special_bonus_imba_centaur_8", "components/abilities/heroes/centaur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_special_bonus_imba_centaur_9", "components/abilities/heroes/centaur", LUA_MODIFIER_MOTION_NONE)
