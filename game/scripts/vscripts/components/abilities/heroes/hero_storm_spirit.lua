...
function imba_storm_spirit_ball_lightning:OnProjectileThink_ExtraData(location, ExtraData)
	...
	if (self.traveled + ExtraData.speed < self.distance) and caster:IsAlive() and (caster:GetMana() > ExtraData.total_mana_cost / 100) then
		...
		caster:ReduceMana(((ExtraData.pct_mana_cost / 100) + ExtraData.base_mana_cost) * self.units_traveled_in_last_tick / 100)
...
function imba_storm_spirit_ball_lightning:OnProjectileHit_ExtraData(target, location, ExtraData)
	if IsServer() then
		if target then
			local caster = self:GetCaster()
			local damage = ExtraData.damage * math.floor(self.traveled / 100)
			...
			if self.traveled > ExtraData.max_spell_amp_range then
				...
				local damage_at_max_distance = ExtraData.damage * ExtraData.max_spell_amp_range / 100 * (1 + (caster:GetSpellAmplification(false) / 100))
...
function imba_storm_spirit_ball_lightning:GetManaCost()
	...
	return (base_cost + (caster:GetMaxMana() * pct_cost / 100))
end
