	...
	if self.premature_modifier then
		...
		elseif self:GetAbility() and self:GetAbility():GetAutoCastState() then
			...
			if self.premature_modifier.store_health_loss then
				spawn_unit:SetBaseMaxHealth(self.premature_base_health + self.premature_modifier.store_health_loss * self.premature_health_pct / 100)
				spawn_unit:SetMaxHealth(self.premature_base_health + self.premature_modifier.store_health_loss * self.premature_health_pct / 100)
				spawn_unit:SetHealth(self.premature_base_health + self.premature_modifier.store_health_loss * self.premature_health_pct / 100)

				spawn_unit:SetBaseDamageMin(self.premature_base_attack_min + self.premature_modifier.store_health_loss * self.premature_attack_pct / 100)
				spawn_unit:SetBaseDamageMax(self.premature_base_attack_max + self.premature_modifier.store_health_loss * self.premature_attack_pct / 100)
			end
...
function modifier_imba_slark_dark_pact_pulses:OnDestroy()
	...
	if self.premature_modifier and not self.premature_modifier:IsNull() and self.premature_modifier:GetStackCount() ~= 0 and self.bStoreHealth then
		...
		self.store_health_loss       = math.max(self.total_damage * 0.5, math.min(self.health_differential, self:GetParent():GetMaxHealth() * self.health_differential_pct / 100))
...
function modifier_imba_slark_pounce_leash:OnCreated(params)
	...
	self.begin_slow_radius = params.leash_radius * 80 / 100
