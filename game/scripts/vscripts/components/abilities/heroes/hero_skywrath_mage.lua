...
function imba_skywrath_mage_arcane_bolt:OnProjectileHit(target, location)
	...
	local int_damage_bonus = intelligence * ((intelligence_bonus_pct + intelligence_bonus_per_stack * stacks) / 100)
...
function imba_skywrath_mage_concussive_shot:OnProjectileHit_ExtraData(target, location, extra_data)
	...
	if primary_concussive == 0 then
		damage = damage * (1 - (ghastly_damage_pct / 100))
	end
	...
	for _, enemy in pairs(enemies) do
		if not enemy:IsMagicImmune() then
			if enemy:IsCreep() then
				final_damage = damage * self:GetSpecialValueFor("creep_damage_pct") / 100
			else
				final_damage = damage
			end
	...
	if IsServer() then
		...
		Timers:CreateTimer(self.damage_duration, function()
			...
			Timers:CreateTimer(self.explosion_delay, function()
				...
				self.explosion_damage = self.caster_int * ((self.int_as_damage_pct + self.int_increase_per_stack * self.wrath_stacks) / 100)
