...
LinkLuaModifier("modifier_imba_clinkz_death_pact_723", "components/abilities/heroes/hero_clinkz",
	LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_clinkz_death_pact_723_enemy", "components/abilities/heroes/hero_clinkz",
	LUA_MODIFIER_MOTION_NONE)
...
function imba_clinkz_death_pact_723:CastFilterResultTarget(hTarget)
	...
	elseif hTarget:GetTeamNumber() ~= self:GetCaster():GetTeamNumber() and hTarget:IsCreep() and not hTarget:IsAncient() and hTarget:GetLevel() > self:GetSpecialValueFor("neutral_level") then
		...
		return UnitFilter(hTarget, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_CREEP,
			DOTA_UNIT_TARGET_FLAG_NOT_ANCIENTS, self:GetCaster():GetTeamNumber())
	end
end
...
function imba_clinkz_death_pact_723:OnSpellStart()
	...
	local pact_particle = ParticleManager:CreateParticle("particles/units/heroes/hero_clinkz/clinkz_death_pact.vpcf",
	...
	if not target:IsConsideredHero() then
		...
		-- IMBAfication: Soul High
		local health_to_convert = target:GetMaxHealth() * self:GetSpecialValueFor("soul_high_hp_damage") / 100
...
function SetArrowAttackProjectile(caster, searing_arrows)
	...
	local searing_deso_skadi_projectile =
	"particles/hero/clinkz/searing_skadi_desolator/searing_skadi_desolator_arrow.vpcf"
	local searing_lifesteal_skadi_projectile =
	"particles/hero/clinkz/searing_skadi_lifesteal/searing_skadi_steal_arrow.vpcf"
	local searing_lifesteal_deso_projectile = "particles/hero/clinkz/searing_deso_lifesteal/searing_deso_lifesteal.vpcf"
	local searing_lifesteal_deso_skadi_projectile =
	"particles/hero/clinkz/searing_skadi_deso_steal/searing_skadi_deso_steal_arrow.vpcf"
...
function modifier_imba_searing_arrows_active:OnCreated()
	...
	if IsServer() then
		...
		self.particle_flame_fx = ParticleManager:CreateParticle(self.particle_flame, PATTACH_POINT_FOLLOW,
			self:GetParent())
		ParticleManager:SetParticleControlEnt(self.particle_flame_fx, 0, self:GetParent(), PATTACH_POINT_FOLLOW,
...
function modifier_imba_searing_arrows_active:OnIntervalThink()
	AddFOWViewer(self:GetCaster():GetTeamNumber(), self:GetParent():GetAbsOrigin(), self.vision_radius, FrameTime(),
		false)
end
...
LinkLuaModifier("modifier_imba_skeleton_walk_talent_root", "components/abilities/heroes/hero_clinkz",
	LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_skeleton_walk_talent_ms", "components/abilities/heroes/hero_clinkz",
	LUA_MODIFIER_MOTION_NONE)
	...
	if scepter then
		if caster:HasModifier(modifier_mount) then
			...
			if modifier_mount_handler then
				...
				mounted_ally:AddNewModifier(caster, self, modifier_invis,
	...
	if IsServer() then
		if self:GetCaster():HasScepter() and self:GetCaster():FindAbilityByName("clinkz_burning_army") and self:GetCaster():FindAbilityByName("clinkz_burning_army"):IsTrained() then
			for i = 1, self:GetAbility():GetSpecialValueFor("scepter_skeleton_count") do
				...
				local archer = CreateUnitByName("npc_dota_clinkz_skeleton_archer", pos, true, self:GetCaster(),
					self:GetCaster(), self:GetCaster():GetTeamNumber())
				archer:AddNewModifier(self:GetCaster(), self:GetCaster():FindAbilityByName("clinkz_burning_army"),
					"modifier_imba_clinkz_burning_army_skeleton_custom", {})
				archer:AddNewModifier(self:GetCaster(), nil, "modifier_kill",
					{ duration = self:GetCaster():FindAbilityByName("clinkz_burning_army"):GetSpecialValueFor("duration") })
		...
		if self:GetCaster():HasTalent("special_bonus_imba_clinkz_6") then
			self:GetParent():AddNewModifier(self:GetCaster(), self:GetAbility(), self.modifier_talent_ms,
				{ duration = self:GetCaster():FindTalentValue("special_bonus_imba_clinkz_6") })
		end
...
function modifier_imba_skeleton_walk_invis:OnDestroy()
	...
	if self:GetAbility() and self:GetAbility():GetName() == "imba_clinkz_skeleton_walk_723" then
		self:GetParent():AddNewModifier(self:GetCaster(), self:GetAbility(),
			...
			{ duration = self:GetAbility():GetTalentSpecialValueFor("attack_speed_duration") })
	end
end
...
function modifier_imba_skeleton_walk_spook:OnCreated()
	if IsServer() then
		...
		-- Add particle
		self.particle_spook_fx = ParticleManager:CreateParticle(self.particle_spook, PATTACH_OVERHEAD_FOLLOW,
			self:GetParent())
...
LinkLuaModifier("modifier_imba_clinkz_skeleton_walk_723_strafe", "components/abilities/heroes/hero_clinkz",
	LUA_MODIFIER_MOTION_NONE)
...
LinkLuaModifier("modifier_imba_death_pact_stack_creep", "components/abilities/heroes/hero_clinkz",
	LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_death_pact_stack_hero", "components/abilities/heroes/hero_clinkz",
	LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_death_pact_talent_debuff", "components/abilities/heroes/hero_clinkz",
	LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_death_pact_talent_buff", "components/abilities/heroes/hero_clinkz",
	LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_death_pact_hero_debuff", "components/abilities/heroes/hero_clinkz",
	LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_death_pact_spirit_aura", "components/abilities/heroes/hero_clinkz",
	LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_death_pact_spirit_attack_range", "components/abilities/heroes/hero_clinkz",
	LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_death_pact_bonus_spirited", "components/abilities/heroes/hero_clinkz",
	LUA_MODIFIER_MOTION_NONE)
...
function imba_clinkz_death_pact:OnSpellStart()
	...
	if target:IsHero() then
		...
		local damage = current_hp * (hero_current_hp_damage_pct / 100)
		...
		local spirit = CreateUnitByName("npc_imba_clinkz_spirits", summon_point, true, caster, caster,
...
function modifier_imba_death_pact_stack_creep:OnCreated()
	...
	ParticleManager:SetParticleControlEnt(self.particle_pact_buff_fx, 0, caster, PATTACH_POINT_FOLLOW, "attach_hitloc",
...
function modifier_imba_death_pact_stack_creep:GetModifierBaseAttack_BonusDamage()
	local stacks = self:GetStackCount()
	local bonus_damage = self.creep_bonus_dmg_pct / 100 * stacks
...
function modifier_imba_death_pact_stack_creep:GetModifierExtraHealthBonus()
	local stacks = self:GetStackCount()
	local bonus_hp = self.creep_bonus_hp_pct / 100 * stacks
...
function modifier_imba_death_pact_spirit_aura:OnCreated()
	if IsServer() then
		...
		Timers:CreateTimer(FrameTime(), function()
			...
			-- Add particle effect
			local particle_pact = "particles/units/heroes/hero_clinkz/clinkz_death_pact.vpcf"
			local particle_pact_fx = ParticleManager:CreateParticle(particle_pact, PATTACH_ABSORIGIN_FOLLOW,
				self:GetParent())
...
function modifier_imba_death_pact_stack_hero:GetModifierBaseAttack_BonusDamage()
	local stacks = self:GetStackCount()
	local bonus_damage = self.hero_bonus_dmg_pct / 100 * stacks
...
function modifier_imba_death_pact_hero_debuff:OnHeroKilled(keys)
	if IsServer() then
		...
		if killed_hero == self:GetParent() and self:GetCaster():HasModifier(modifier_stack_hero) then
			...
			local spirit = CreateUnitByName("npc_imba_clinkz_spirits", summon_point, true, self:GetCaster(),
				self:GetCaster(), self:GetCaster():GetTeamNumber())
			...
			-- Add Spirit's Attack Damage					
			local modifier_spirit_attack_range_bonus = "modifier_imba_death_pact_spirit_attack_range"
			spirit:AddNewModifier(self:GetCaster(), self:GetAbility(), modifier_spirit_attack_range_bonus,
				{ duration = duration })

			-- ALL YOUR STATS ARE BELONG TO CLINKZ
			self:GetCaster():AddNewModifier(self:GetParent(), self:GetAbility(), modifier_bonus_spirited,
				{ duration = duration })
		end
	end
end
...
LinkLuaModifier("modifier_special_bonus_imba_clinkz_8", "components/abilities/heroes/hero_clinkz",
	LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_special_bonus_imba_clinkz_death_pact_723_health", "components/abilities/heroes/hero_clinkz",
	LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_special_bonus_imba_clinkz_skeleton_walk_723_strafe_duration",
	"components/abilities/heroes/hero_clinkz", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_special_bonus_imba_clinkz_9", "components/abilities/heroes/hero_clinkz",
	LUA_MODIFIER_MOTION_NONE)
...
function modifier_imba_burning_army:OnAttackLanded(params)
	...
	if IsServer() and attacker == self:GetParent() and caster:HasTalent("special_bonus_imba_clinkz_8") then
		...
		local particle_manaburn_fx = ParticleManager:CreateParticle("particles/generic_gameplay/generic_manaburn.vpcf",
			PATTACH_ABSORIGIN_FOLLOW, target)
