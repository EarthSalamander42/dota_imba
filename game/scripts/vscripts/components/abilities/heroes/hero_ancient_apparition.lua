...
LinkLuaModifier("modifier_imba_ancient_apparition_cold_feet", "components/abilities/heroes/hero_ancient_apparition",
	LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_ancient_apparition_cold_feet_freeze",
	"components/abilities/heroes/hero_ancient_apparition", LUA_MODIFIER_MOTION_NONE)

LinkLuaModifier("modifier_imba_ancient_apparition_ice_vortex_thinker",
	"components/abilities/heroes/hero_ancient_apparition", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_ancient_apparition_ice_vortex", "components/abilities/heroes/hero_ancient_apparition",
	LUA_MODIFIER_MOTION_NONE)

LinkLuaModifier("modifier_generic_orb_effect_lua", "components/modifiers/generic/modifier_generic_orb_effect_lua",
	LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_ancient_apparition_chilling_touch_slow",
	"components/abilities/heroes/hero_ancient_apparition", LUA_MODIFIER_MOTION_NONE)

LinkLuaModifier("modifier_imba_ancient_apparition_imbued_ice", "components/abilities/heroes/hero_ancient_apparition",
	LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_ancient_apparition_imbued_ice_slow", "components/abilities/heroes/hero_ancient_apparition",
	LUA_MODIFIER_MOTION_NONE)

LinkLuaModifier("modifier_imba_ancient_apparition_anti_abrasion_thinker",
	"components/abilities/heroes/hero_ancient_apparition", LUA_MODIFIER_MOTION_NONE)

LinkLuaModifier("modifier_imba_ancient_apparition_ice_blast_thinker",
	"components/abilities/heroes/hero_ancient_apparition", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_ancient_apparition_ice_blast", "components/abilities/heroes/hero_ancient_apparition",
	LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_ancient_apparition_ice_blast_global_cooling",
	"components/abilities/heroes/hero_ancient_apparition", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_ancient_apparition_ice_blast_cold_hearted",
	"components/abilities/heroes/hero_ancient_apparition", LUA_MODIFIER_MOTION_NONE)
...
function imba_ancient_apparition_cold_feet:OnSpellStart()
	if not self:GetCaster():HasTalent("special_bonus_imba_ancient_apparition_cold_feet_aoe") then
		...
		local enemies = FindUnitsInRadius(self:GetCaster():GetTeamNumber(), self:GetCursorPosition(), nil,
			self:GetCaster():FindTalentValue("special_bonus_imba_ancient_apparition_cold_feet_aoe"),
		...
		if self.counter >= 1 then
			if self.ticks < self.duration then
				...
				self:GetParent():AddNewModifier(self:GetCaster(), self:GetAbility(),
					"modifier_imba_ancient_apparition_cold_feet_freeze",
					{ duration = self.stun_duration * (1 - self:GetParent():GetStatusResistance()) })
...
function modifier_imba_ancient_apparition_cold_feet_freeze:OnOrder(keys)
	if keys.unit:GetTeamNumber() == self:GetParent():GetTeamNumber() and keys.target == self:GetParent() and keys.unit ~= self:GetParent() and not keys.unit:IsMagicImmune() then
		if not keys.unit:HasModifier("imba_ancient_apparition_cold_feet") then
			keys.unit:AddNewModifier(self:GetCaster(), self:GetAbility(),
				"modifier_imba_ancient_apparition_cold_feet_freeze", { duration = self:GetRemainingTime() })
		end
	end
end
...
function modifier_imba_ancient_apparition_chilling_touch_slow:CheckState()
	...
	return {
		[MODIFIER_STATE_FROZEN] = self:GetElapsedTime() <= self.packed_ice_duration * (1 - self:GetParent():GetStatusResistance()) }
end
...
function imba_ancient_apparition_imbued_ice:OnSpellStart()
	...
	ParticleManager:SetParticleControl(imbued_ice_particle, 1,
		Vector(self:GetSpecialValueFor("radius"), self:GetSpecialValueFor("radius"), 0))
	...
	self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_imba_ancient_apparition_imbued_ice",
		{ duration = self:GetSpecialValueFor("buff_duration") })

	local allies = FindUnitsInRadius(self:GetCaster():GetTeamNumber(), position, nil, self:GetSpecialValueFor("radius"),
		DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_NOT_ILLUSIONS, FIND_ANY_ORDER, false)

	for _, ally in pairs(allies) do
		if ally ~= self:GetCaster() then
			ally:AddNewModifier(self:GetCaster(), self, "modifier_imba_ancient_apparition_imbued_ice",
				{ duration = self:GetSpecialValueFor("buff_duration") })
		end
	end
end
...
function modifier_imba_ancient_apparition_imbued_ice:OnAttackLanded(keys)
	if keys.attacker == self:GetParent() and not keys.target:IsMagicImmune() and not keys.target:IsBuilding() then
		...
		keys.target:AddNewModifier(self:GetCaster(), self:GetAbility(),
			"modifier_imba_ancient_apparition_imbued_ice_slow",
			{ duration = self.move_speed_duration * (1 - keys.target:GetStatusResistance()) })
...
function imba_ancient_apparition_anti_abrasion:OnSpellStart()
	...
	local vortex_thinker = CreateModifierThinker(self:GetCaster(), self,
		"modifier_imba_ancient_apparition_anti_abrasion_thinker",
...
function imba_ancient_apparition_ice_blast:OnSpellStart()
	...
	self.ice_blast_dummy = CreateModifierThinker(self:GetCaster(), self,
		"modifier_imba_ancient_apparition_ice_blast_thinker", { x = velocity.x, y = velocity.y },
...
function imba_ancient_apparition_ice_blast_release:OnSpellStart()
	...
	if self.ice_blast_ability then
		if self.ice_blast_ability.ice_blast_dummy and self.ice_blast_ability.initial_projectile then
			...
			-- CP5: Duration
			ParticleManager:SetParticleControl(ice_blast_particle, 5,
				Vector(math.min(vector:Length2D() / velocity:Length2D(), 2), 0, 0))
			ParticleManager:ReleaseParticleIndex(ice_blast_particle)
			...
			AddFOWViewer(self:GetCaster():GetTeamNumber(), self.ice_blast_ability.ice_blast_dummy:GetAbsOrigin(), 650, 4,
				false)
	...
	if self.ice_blast_ability then
		...
		for _, enemy in pairs(enemies) do
			if enemy:IsInvulnerable() then
				...
				enemy:AddNewModifier(self:GetCaster(), self.ice_blast_ability,
...
function imba_ancient_apparition_ice_blast_release:OnProjectileHit_ExtraData(target, location, data)
	if not target and self.ice_blast_ability then
		...
		local damage               = GetAbilityKV(vanilla_ability_name, "AbilityDamage",
			self.ice_blast_ability:GetLevel())
		...
		for _, enemy in pairs(enemies) do
			...
			self:GetCaster():AddNewModifier(self:GetCaster(), self.ice_blast_ability,
				"modifier_imba_ancient_apparition_ice_blast_cold_hearted",
				{ duration = duration, regen = enemy:GetHealthRegen() })
		end
...
LinkLuaModifier("modifier_special_bonus_imba_ancient_apparition_chilling_touch_range",
	"components/abilities/heroes/hero_ancient_apparition", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_special_bonus_imba_ancient_apparition_ice_vortex_cooldown",
	"components/abilities/heroes/hero_ancient_apparition", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_special_bonus_imba_ancient_apparition_chilling_touch_damage",
	"components/abilities/heroes/hero_ancient_apparition", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_special_bonus_imba_ancient_apparition_ice_vortex_boost",
	"components/abilities/heroes/hero_ancient_apparition", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_special_bonus_imba_ancient_apparition_ice_blast_kill_threshold",
	"components/abilities/heroes/hero_ancient_apparition", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_special_bonus_imba_ancient_apparition_cold_feet_aoe",
	"components/abilities/heroes/hero_ancient_apparition", LUA_MODIFIER_MOTION_NONE)
...
function imba_ancient_apparition_cold_feet:OnOwnerSpawned()
	if self:GetCaster():HasTalent("special_bonus_imba_ancient_apparition_cold_feet_aoe") and not self:GetCaster():HasModifier("modifier_special_bonus_imba_ancient_apparition_cold_feet_aoe") then
		self:GetCaster():AddNewModifier(self:GetCaster(),
			self:GetCaster():FindAbilityByName("special_bonus_imba_ancient_apparition_cold_feet_aoe"),
			"modifier_special_bonus_imba_ancient_apparition_cold_feet_aoe", {})
	end
end
...
function imba_ancient_apparition_ice_vortex:OnOwnerSpawned()
	if self:GetCaster():HasTalent("special_bonus_imba_ancient_apparition_ice_vortex_cooldown") and not self:GetCaster():HasModifier("modifier_special_bonus_imba_ancient_apparition_ice_vortex_cooldown") then
		self:GetCaster():AddNewModifier(self:GetCaster(),
			self:GetCaster():FindAbilityByName("special_bonus_imba_ancient_apparition_ice_vortex_cooldown"),
			"modifier_special_bonus_imba_ancient_apparition_ice_vortex_cooldown", {})
	end
	...
	if self:GetCaster():HasTalent("special_bonus_imba_ancient_apparition_ice_vortex_boost") and not self:GetCaster():HasModifier("modifier_special_bonus_imba_ancient_apparition_ice_vortex_boost") then
		self:GetCaster():AddNewModifier(self:GetCaster(),
			self:GetCaster():FindAbilityByName("special_bonus_imba_ancient_apparition_ice_vortex_boost"),
			"modifier_special_bonus_imba_ancient_apparition_ice_vortex_boost", {})
	end
end
...
function imba_ancient_apparition_chilling_touch:OnOwnerSpawned()
	if self:GetCaster():HasTalent("special_bonus_imba_ancient_apparition_chilling_touch_range") and not self:GetCaster():HasModifier("modifier_special_bonus_imba_ancient_apparition_chilling_touch_range") then
		self:GetCaster():AddNewModifier(self:GetCaster(),
			self:GetCaster():FindAbilityByName("special_bonus_imba_ancient_apparition_chilling_touch_range"),
			"modifier_special_bonus_imba_ancient_apparition_chilling_touch_range", {})
	end
	...
	if self:GetCaster():HasTalent("special_bonus_imba_ancient_apparition_chilling_touch_damage") and not self:GetCaster():HasModifier("modifier_special_bonus_imba_ancient_apparition_chilling_touch_damage") then
		self:GetCaster():AddNewModifier(self:GetCaster(),
			self:GetCaster():FindAbilityByName("special_bonus_imba_ancient_apparition_chilling_touch_damage"),
			"modifier_special_bonus_imba_ancient_apparition_chilling_touch_damage", {})
	end
end
