		...
		if target:IsAlive() then
			...
			if (not target:IsSilenced() or not target:FindModifierByName("modifier_imba_silencer_global_silence") or not target:HasModifier("modifier_imba_silencer_global_silence_v2")) or self.aghs_upgraded then
				...
				if self.talent_learned then
					local heal_amount = actual_Damage * self.caster:FindTalentValue("special_bonus_imba_silencer_1") / 100
	...
	if IsServer() then
		...
		if self.caster == attacker then
			if target:IsAlive() and self.glaive_attack then
				...
				-- glaive_pure_damage = glaive_pure_damage * (1 + (self.scepter_damage_multiplier / 100))
				-- end
...
function modifier_imba_silencer_global_silence_v2_mana_reduction:OnCreated(keys)
	...
	self:SetStackCount(self:GetParent():GetMaxMana() * (self.max_mana_reduction_percentage) / 100 * (-1))
end
