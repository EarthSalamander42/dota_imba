...
function modifier_imba_manadrain_aura_debuff:OnIntervalThink()
	if IsServer() and not self.caster:IsNull() then
		...
		local mana_drain_per_sec = self.parent:GetMaxMana() * self.aura_mana_drain_pct / 100
...
function modifier_imba_manadrain_debuff:OnIntervalThink()
	if IsServer() then
		...
		local damage = self.mana_drained * self.mana_pct_as_damage / 100
...
function imba_lion_finger_of_death:GetManaCost(level)
	...
	local mana_cost = base_mana_cost * (1 + (stacks * triggerfinger_mana_inc_pct / 100))
	return mana_cost
end
