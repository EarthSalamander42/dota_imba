	...
	if IsServer() then
		...
		if self.parent == attacker and target:GetTeamNumber() ~= self.parent:GetTeamNumber() then
			...
			if self:GetParent():IsIllusion() then
				target_mana_burn = target_mana_burn * self:GetAbility():GetSpecialValueFor("illusion_percentage") / 100
			end
...
function modifier_imba_mana_break_passive:OnAttackLanded(keys)
	if IsServer() then
		...
		if target:GetTeamNumber() ~= attacker:GetTeamNumber() then
			...
			local manaburn_pfx = ParticleManager:CreateParticle("particles/generic_gameplay/generic_manaburn.vpcf",
			...
			if current_mana_phase < self.mana_phase or current_mana_phase == 0 then
				-- Sexy blast effect
				local particle_blast_fx = ParticleManager:CreateParticle(self.particle_blast, PATTACH_CUSTOMORIGIN_FOLLOW,
					target)
				ParticleManager:SetParticleControlEnt(particle_blast_fx, 0, target, PATTACH_POINT_FOLLOW, "attach_hitloc",
					target:GetAbsOrigin(), true)
				ParticleManager:ReleaseParticleIndex(particle_blast_fx)

				-- Calculate blast damage
				local blast_damage = target:GetMaxMana() * self.max_mana_blast / 100
				...
				if self.parent:IsIllusion() then
					blast_damage = blast_damage * self.illusions_efficiency_pct / 100
				end
			...
			if attacker:HasTalent("special_bonus_imba_antimage_7") then
				...
				-- Create particle
				local particle_aoe_mana_burn_fx = ParticleManager:CreateParticle(self.particle_aoe_mana_burn,
					PATTACH_ABSORIGIN, attacker)
				...
				-- Calculate mana to burn
				local mana_aoe_break = target_mana_burn * mana_burn_pct / 100
...
function imba_antimage_blink:OnSpellStart()
	...
	local blink_pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_antimage/antimage_blink_start.vpcf",
	...
	Timers:CreateTimer(0.01, function()
		...
		if ability.percent_mana_burn ~= 0 then
			...
			local nearby_enemies = FindUnitsInRadius(caster:GetTeamNumber(), caster:GetAbsOrigin(), nil, ability.radius,
...
function imba_antimage_mana_void:OnSpellStart()
	...
	Timers:CreateTimer(time_to_wait, function()
		...
		local nearby_enemies = FindUnitsInRadius(caster:GetTeamNumber(), target:GetAbsOrigin(), nil, radius,
			DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_NONE,
			FIND_ANY_ORDER, false)
		for _, enemy in pairs(nearby_enemies) do
			-- Calculate this enemy's damage contribution
		...
		local void_pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_antimage/antimage_manavoid.vpcf",
			PATTACH_POINT_FOLLOW, target)
		ParticleManager:SetParticleControlEnt(void_pfx, 0, target, PATTACH_POINT_FOLLOW, "attach_hitloc",
			target:GetOrigin(), true)
...
LinkLuaModifier("modifier_special_bonus_imba_antimage_8", "components/abilities/heroes/hero_antimage",
	LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_special_bonus_imba_antimage_9", "components/abilities/heroes/hero_antimage",
	LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_special_bonus_imba_antimage_10", "components/abilities/heroes/hero_antimage",
	LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_special_bonus_imba_antimage_11", "components/abilities/heroes/hero_antimage",
	LUA_MODIFIER_MOTION_NONE)
...
function imba_antimage_blink:OnOwnerSpawned()
	if self:GetCaster():HasTalent("special_bonus_imba_antimage_10") and not self:GetCaster():HasModifier("modifier_special_bonus_imba_antimage_10") then
		self:GetCaster():AddNewModifier(self:GetCaster(),
			self:GetCaster():FindAbilityByName("special_bonus_imba_antimage_10"),
			"modifier_special_bonus_imba_antimage_10", {})
	end
	...
	if self:GetCaster():HasTalent("special_bonus_imba_antimage_blink_range") and not self:GetCaster():HasModifier("modifier_special_bonus_imba_antimage_blink_range") then
		self:GetCaster():AddNewModifier(self:GetCaster(),
			self:GetCaster():FindAbilityByName("special_bonus_imba_antimage_blink_range"),
			"modifier_special_bonus_imba_antimage_blink_range", {})
	end
end
...
function imba_antimage_spell_shield:OnOwnerSpawned()
	if self:GetCaster():HasTalent("special_bonus_imba_antimage_11") and not self:GetCaster():HasModifier("modifier_special_bonus_imba_antimage_11") then
		self:GetCaster():AddNewModifier(self:GetCaster(),
			self:GetCaster():FindAbilityByName("special_bonus_imba_antimage_11"),
			"modifier_special_bonus_imba_antimage_11", {})
	end
end
