...
function imba_earth_spirit_stone_caller:OnSpellStart()
	...
	local remnantDuration = self:GetSpecialValueFor("duration")
	local effectRadius = self:GetSpecialValueFor("radius")
...
function imba_earth_spirit_stone_caller:OnUpgrade()
	if IsServer() then self:CheckScepter() end
end
	...
	if IsServer() then
		...
		if self:GetParent():GetUnitName() == "npc_imba_dota_earth_spirit_stone" then
			...
			if self.PetrifyHandler then
				...
				local damageRadius = self.PetrifyHandler:GetSpecialValueFor("aoe")
...
function modifier_imba_earth_spirit_layout_fix:DeclareFunctions()
	return { MODIFIER_PROPERTY_ABILITY_LAYOUT }
end
...
function imba_earth_spirit_boulder_smash:GetCastRange()
	if IsClient() then
		...
		return self:GetSpecialValueFor("rock_distance")
	end
end
...
function imba_earth_spirit_boulder_smash:OnAbilityPhaseStart()
	if IsServer() then
		...
		local caster = self:GetCaster()
		local searchRadius = self:GetSpecialValueFor("rock_search_aoe") + GetCastRangeIncrease(caster)
		...
		if target then
			...
			if CalcDistanceBetweenEntityOBB(caster, target) <= searchRadius then
				...
				if caster:GetTeamNumber() ~= target:GetTeamNumber() then
					ApplyDamage({ victim = target, attacker = caster, damage = self:GetSpecialValueFor("rock_damage"), damage_type = DAMAGE_TYPE_MAGICAL, ability = self })
				end
...
function modifier_imba_boulder_smash_push:OnCreated()
	if IsServer() then
		...
		Timers:CreateTimer(FrameTime(), function()
			...
			-- ability params
			self.hitRadius = self.ability:GetSpecialValueFor("radius")
			self.damage = self.ability:GetSpecialValueFor("rock_damage")
			self.velocity = self.ability:GetSpecialValueFor("speed")
			self.debuff_duration = self.ability:GetSpecialValueFor("duration")
			local remnantDistance = self.ability:GetSpecialValueFor("rock_distance")
...
function modifier_imba_boulder_smash_cast_thinker:OnCreated()
	if IsServer() then
		...
		self.castRange = self:GetAbility():GetSpecialValueFor("rock_search_aoe") + GetCastRangeIncrease(self.caster) - 20
...
function modifier_imba_rolling_boulder:OnCreated()
	if IsServer() then
		...
		self.remnantDistance = self.ability:GetSpecialValueFor("rock_distance")
		self.remnantVelocity = self.ability:GetSpecialValueFor("rock_speed")
			...
			if not self.hitRemnant then
				...
				for _, r in ipairs(RemnantFinder) do
					if r:HasModifier("modifier_imba_stone_remnant") then
						...
						self.stunDuration = self.stunDuration + self.ability:GetSpecialValueFor("rock_bonus_duration")
						break
					end
				end
			end
...
function modifier_imba_rolling_boulder_slow:OnCreated()
	self.slowPcnt = self:GetAbility():GetSpecialValueFor("move_slow") * -1
end

function modifier_imba_rolling_boulder_slow:GetModifierMoveSpeedBonus_Percentage()
	return self.slowPcnt
end
...
function imba_earth_spirit_geomagnetic_grip:OnAbilityPhaseStart()
	if IsServer() then
		local remnantSearchRadius = self:GetSpecialValueFor("radius")
		local target = self:GetCursorTarget()
		local pointTarget = self:GetCursorPosition()
		local caster = self:GetCaster()
...
function modifier_imba_geomagnetic_grip_pull:SetRealCaster(rc)
	if IsServer() then self.caster = rc end
end
...
function modifier_imba_geomagnetic_grip_pull:OnCreated()
	if IsServer() then
		...
		Timers:CreateTimer(FrameTime(), function()
			...
			self.hitRadius = self.ability:GetSpecialValueFor("radius")
			self.damage = self.ability:GetSpecialValueFor("rock_damage")
			self.silenceDuration = self.ability:GetSpecialValueFor("duration")
			self.normalVelocity = self.ability:GetSpecialValueFor("speed")
...
function imba_earth_spirit_magnetize:OnSpellStart()
	if IsServer() then
		local searchRadius = self:GetSpecialValueFor("rock_search_radius")
		local debuffDuration = self:GetSpecialValueFor("damage_duration")
		local caster = self:GetCaster()
...
function modifier_imba_magnetize:OnCreated()
	if IsServer() then
		...
		self.tickDamge = self.ability:GetSpecialValueFor("damage_per_second") * self.tickInterval
		self.refreshRadius = self.ability:GetSpecialValueFor("rock_explosion_radius")
		self.remnantSearchRadius = self.ability:GetSpecialValueFor("rock_search_radius")
		self.baseDration = self.ability:GetSpecialValueFor("damage_duration")
		self.remnantNewLifespan = self.ability:GetSpecialValueFor("rock_explosion_delay")
