...
function modifier_imba_tiny_death_handler:DeclareFunctions()
	return {
	...
	if IsServer() then
		...
		for _, victim in pairs(victims) do
			...
			if victim:IsBuilding() then
				damage = damage * self.ability:GetSpecialValueFor("building_dmg") / 100
		...
		if self.parent == caster then
			ApplyDamage({ victim = caster, attacker = caster, damage = caster:GetMaxHealth() * self.ability:GetSpecialValueFor("self_dmg_pct") / 100, damage_type = DAMAGE_TYPE_PURE, damage_flags = DOTA_DAMAGE_FLAG_NO_SPELL_AMPLIFICATION + DOTA_DAMAGE_FLAG_NO_SPELL_LIFESTEAL + DOTA_DAMAGE_FLAG_REFLECTION + DOTA_DAMAGE_FLAG_HPLOSS, ability = self.ability })
		end
...
function modifier_tiny_toss_scepter_bounce:OnRemoved()
	if IsServer() then
		local damage = self.toss_damage * self.scepter_bounce_damage_pct / 100
