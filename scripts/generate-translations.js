...
//			console.log(key, 'is unchanged from', unchagedKeys[key]);
	...
	fs.writeFileSync(path.join(__dirname, '../', settings.game_path, '/resource/addon_english.txt'), '\ufeff' + englishFileString, {
		encoding: 'ucs2'
	});
...
function generateTranslations (lang) {
	getTranslationsForLanguage(languageShortNames[lang], function (err, data) {
		...
		generateFileForTranslations(lang, data, function (lines) {
			fs.writeFileSync(path.join(__dirname, '../', settings.game_path, 'resource/addon_' + lang + '.txt'), '\ufeff' + lines.join('\n'), {
				encoding: 'ucs2'
			});
		});
	});
