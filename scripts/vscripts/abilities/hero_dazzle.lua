...
function ShallowGravePassive( keys )
	...
	if ability:GetCooldownTimeRemaining() == 0 then
		ability:ApplyDataDrivenModifier( caster, caster, "modifier_shallow_grave_scepter", {})
	else
		caster:RemoveModifierByName("modifier_shallow_grave_scepter")
	end
end

function ShallowGraveDamage( keys )
	local caster = keys.caster
	local ability = keys.ability
	...
	if health == 1 then
		ability:ApplyDataDrivenModifier( caster, caster, "modifier_shallow_grave", {})
...
function ShallowGraveHeal( keys )
	...
	caster:Heal(unit.grave_damage, caster)
	unit.grave_damage = 0
end
...
function ShadowWave( keys )
	...
	local armor_bonus = "modifier_armor_bonus"
	local armor_reduction = "modifier_armor_reduction"
	...
	if target ~= caster then
		...
		for _,v in pairs(units_to_damage) do
			ability:ApplyDataDrivenModifier(caster, v, armor_reduction, {})
			-- Play the particle
