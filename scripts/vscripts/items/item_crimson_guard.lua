function crimson_guard_activate( keys )
	...
	local meleeModifier = "modifier_crimson_guard_active_melee"
	local rangedModifier = "modifier_crimson_guard_avtive_ranged"
	local cooldownModifier = "modifier_crimson_guard_cooldown"
...
function crimson_guard_created( keys )
	local caster = keys.caster
	local ability = keys.ability
	local meleeModifier = "modifier_crimson_guard_block_melee"
	local rangedModifier = "modifier_crimson_guard_block_ranged"

	Timers:CreateTimer( 0.1, function()
		if caster:GetAttackCapability() == DOTA_UNIT_CAP_MELEE_ATTACK then
			caster:RemoveModifierByName("modifier_crimson_guard_block_ranged")
		end
