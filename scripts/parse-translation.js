...
module.exports = function (shouldParse, languageFolder, dotaLanguage) {
  languageFolder = languageFolder || path.join(basePath, settings.game_path, 'resource/English/');
      ...
      try {
        ...
        if (fileData.toString().match(new RegExp('[^\x00-\x7F]'))) { // eslint-disable-line no-control-regex
          fileData.toString().split('\n').forEach(function (line) {
            if (line.match(new RegExp('[^\x00-\x7F]'))) { // eslint-disable-line no-control-regex
              console.error('Warning, this line might break everything:', line);
            }
          });
        }
