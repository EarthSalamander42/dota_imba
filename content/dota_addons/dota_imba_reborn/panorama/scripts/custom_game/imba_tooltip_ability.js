...
//	$.Msg("-------------------")
//	$.Msg(SetTooltipsValues(ability_name, AbilityDescription, AbilityAttributes, true))
...
//				$.Msg("Replace " + "%" + special_key + "%" + " with: " + special_values)
			...
			if ($.Localize("DOTA_Tooltip_Ability_" + ability_name + special_key)) {
				AbilityExtraDescription = AbilityExtraDescription + $.Localize("DOTA_Tooltip_Ability_" + ability_name + "_" + special_key) + " " + special_values + "<br>";
			}
		}
	}
...
*/
	...
	var cd = AbilityValues["cooldown"][ability_index].split(" ");
	...
	if (Abilities.GetCooldown(test_ability) != 0) {
	...
	var mana = AbilityValues["manacost"][ability_index];
